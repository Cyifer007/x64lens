...
def validate(value:Any)->dict[str,Any]:
    ...
    require(value['schema']=='x64lens-sprint13-lifecycle-denominator-authority-v1' and value['sprint']==13 and value['patch']==85,'authority identity changed')
    ...
    require(isinstance(deltas,list) and [d['patch'] for d in deltas]==[80,81,82,83,84],'successor patch sequence changed')
    ...
    required={
        ...
        'abi_role_queries':36,
        'batch_transaction_cases':29,
        'batch_transaction_executions':87,
        'candidate_capacity':4096,
        'diagnostic_x64lens_rows':60,
        'dynamic_lookup_confirmations':64,
        'natural_coordinate_cells':9,
        'natural_coordinate_controls':108,
        'natural_coordinate_executions':48,
        'natural_coordinate_targets':12,
        'producer_generations':3,
        'producer_ordered_pair_observations':90,
        'producer_score_null_cells':75,
        'role_property_combined_private_parity_fields':10368,
        'role_property_combined_public_parity_closures':768,
        'role_property_eligible_matches':1224,
        'role_property_field_dispositions':1728,
        'role_property_objects':96,
        'role_property_private_probe_executions':288,
        'role_property_public_command_executions':384,
        'role_property_readelf_processes':384,
        'workload_confirmation_batches':162,
        'workload_confirmation_members':3402,
    }
...
def mutation_rejections(value:dict[str,Any])->int:
    ...
    # This floor was present in the authority but omitted by the P084
    # validator.  It is an explicit independent mutation so future validators
    # cannot silently regress to a selected subset of the 24-floor contract.
    m=copy.deepcopy(value); m['denominator_floors']['role_property_readelf_processes']=383; mutations.append(m)
    rejected=0
    for mutation in mutations:
        try: validate(mutation)
        except LifecycleError: rejected+=1
        else: raise LifecycleError('lifecycle mutation was accepted')
    require(rejected==13,'mutation denominator changed')
    return rejected
...
def main()->int:
    ...
    print('sprint13-lifecycle-denominator-smoke: ok roots=20 leaves=30 aliases=29 folds=2 tombstones=15 lineage=161 events=87 successor_deltas=5 floors=24 mutations=13 event88=0 new_ids=0')
    return 0
