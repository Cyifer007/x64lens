...
python3 "$(dirname "$0")/validate-json-report.py" \
    --mode system --require-schema 0.2.0 --expected-command gadgets --require-provenance --require-sprint10-effects \
    "$exact_gadgets_json" >/dev/null
python3 "$(dirname "$0")/validate-json-report.py" \
    --mode system --require-schema 0.2.0 --expected-command analyze --require-provenance --require-sprint10-effects \
