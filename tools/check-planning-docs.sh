...
grep -q '^Patch 084 corrective and exact-source acceptance candidate, pending complete acceptance' docs/sprints/sprint-12-plan.md \
    || fail 'Sprint 12 is not marked as the Patch 084 corrective and exact-source acceptance candidate'
grep -q '^Patch 084 correction and private ABI-role contract candidate; activation pending complete Patch 084 acceptance' docs/sprints/sprint-13-plan.md \
    || fail 'Sprint 13 is not marked as the Patch 084 correction and private ABI-role contract candidate'
...
grep -q 'sprint-10-patch-054-validation.md' docs/sprints/sprint-10-plan.md \
    || fail 'Sprint 10 plan does not link Patch 054 validation'
grep -q 'sprint-10-retro.md' docs/sprints/sprint-10-plan.md \
    || fail 'Sprint 10 plan does not link the retrospective'
...
grep -qi 'stack adjustment' docs/adr/0034-bounded-stack-adjust-and-public-artifact-content-policy.md \
    || fail 'ADR 0034 does not record the bounded stack-adjust decision'
...
grep -q 'sprint10-stack-adjust-smoke' docs/sprints/sprint-10-patch-048-validation.md \
...
grep -q '^public-bundle-content-check:' Makefile \
    || fail 'Makefile does not define public-bundle-content-check'
...
grep -q '^research-roadmap-consistency-smoke:' Makefile \
