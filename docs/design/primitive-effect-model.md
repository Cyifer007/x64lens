...
A duplicate pair or a pair whose leading pop is outside the supported set falls
back to the existing single-pop suffix immediately before `ret`. This preserves
a justified fact without overpromoting the longer byte sequence.

## Clobber facts

`clobbers` is a separate register set from `controls`.

- A controlled register receives attacker- or caller-provided data under the
  represented primitive model.
- A clobbered register is modified without a justified controlled value.

Patch 046 introduces the machine-readable field but does not populate a
non-empty clobber set. Future transfer and memory families must define clobbers
before semantic promotion.
...
The scoring engine consumes validated semantic facts but remains an independent
module. The first multi-pop family intentionally has `score: null`. A later
