...
entry candidate and becomes active only after complete Patch 089 acceptance.
...
accepted. Independent exact-source Patch 089 acceptance remains pending. These patches add no public
...
required the Patch 079 corrective and private task-value candidate, whose review
required Patch 080; Patch 080 was superseded by Patch 081, which was not
accepted. Independent exact-source Patch 089 acceptance remains pending. Sprint
...
Patch 087 was the implementation candidate at tree
`47a4ee9868914abc1736ed1ccc76515c0d46f676`. It retained the Patch 086 runtime
and public contracts while implementing candidate corrections for transaction
topology, wrapper signals, source-recovery and custody publication, replay
runtime/package closure authority, terminal attribution, ABI result
publication/source binding, and delivery closure. It did not complete
exact-source acceptance and was superseded by Patch 088.
...
concurrency, or optimization claims. Patch 087 did not complete independent
exact-source acceptance; Patch 089 carries the current acceptance boundary.

## Current Sprint 13 Patch 089 validation

Patch 089 adds `make patch088-corrective-regression-smoke` and
`make sprint13-p089-acceptance-smoke`. The focused correction target validates
predecessor/current replay identity separation, exact Python closure authority,
split-debug producer/result custody, distinct behavior and symbol denominators,
workload fixture uniqueness, and streaming source-recovery limits.

The split-debug target requires the authenticated candidate tree, build tools,
`objcopy`, `nm`, and `addr2line`; it consumes a retained same-tree producer
result or creates an ephemeral one when none is supplied. A successful result
remains opt-in diagnostic evidence and does not authorize default product
packaging.

The acceptance target additionally requires fresh producer evidence, strict
ShellCheck, Docker, replay, ABI, external-natural acquisition, both parity
