...
- Corrects Patch 089 split-debug, producer, replay, recovery, bounded-manifest/helper, evidence, and delivery authorities without changing analyzer runtime or public schema.
- Adds an 80-execution reference-only workload qualification preflight. Its rows may decide whether the later paired phase-attribution matrix should run, but cannot be reused or cited as performance evidence.
- Preserves zero public fields, semantic changes, score changes, decoder activation, concurrency activation, and split-debug product adoption.
- The artifact-backed 80-run preflight qualified fewer than 6/8 fixtures and retires phase instrumentation for the current workload set; fresh local confirmation remains pending.
...
- Retain split-debug as an opt-in diagnostic release experiment only: the Patch 088 runtime tier shrank 66.09%, but runtime plus companion produced no total-transfer saving, post-link redaction did not establish path-stable DWARF, and default product adoption remains unauthorized.
- Classify the retained Patch 088 campaign as comparison-unqualified: every x64lens timing was below its floor and no coordinate cell had a positive anchor, so no performance, RSS, parity, coverage, superiority, or publication claim follows.
...
- Record Patch 088 as a historical implementation candidate that did not complete exact-source acceptance and was superseded by Patch 089; this entry does not record product acceptance.
...
- Preserve the frozen workload/phase authority without claiming it has run or selected an optimization.
- Add no analyzer runtime, public field, semantic class, score, schema, decoder, concurrency, capacity, or output change; split-debug product adoption remains unauthorized.
...
- Keep replay-v2, terminal-attribution, ABI-vector, and paired workload/phase authorities private and diagnostic.
- Freeze the eight-fixture, two-profile paired workload/phase method over 160 planned executions; execution remains deferred and the method selects no optimization.
- Add no analyzer runtime, public field, semantic class, score, schema, decoder, concurrency, capacity, or output change, and authorize no speed, RSS, comparative-coverage, baseline-equivalence, or exploitability claim.
...
- Harden the existing Patch 084 Git-less source, recovery, lifecycle, ABI-query,
  natural-campaign, exact-tree, and delivery validation authorities. No tracked
  file under `src/`, `include/`, or `schemas/` changes; analyzer runtime, CLI
  behavior, schema `0.2.0`, public fields, semantics, scores, mitigation meanings,
  and capacity remain unchanged.
- Enforce all 24 lifecycle denominator floors and retain an explicit Patch 084
  zero-count successor delta; event 88 and new canonical identifiers remain
  unauthorized.
- Require each future ABI closure to use 24 development and 12 semantically
  disjoint confirmation queries, one pinned analyzer copy, per-command target
  reauthentication, and no-replace evidence publication.
- Add Patch 085's first new evidence authority: frozen-input, no-reroll replay
  over the exact twelve predecessor natural-target hashes and five retained tool
  identities. Four execution tools run once per target for 48 slots. Actual
  replay remains pending; any output remains diagnostic, `frozen=false`, and
  publication-ineligible.
- Add Patch 085's second new evidence authority: one private terminal reason at
  each execution, relation, observation, and cell layer. Preserve the predecessor
  counts of 41 successes, five bounded unsupported exits, one timeout, one output-
  limit termination, 48 relation outcomes, 36 observations, and nine cells without
  reinterpretation.
- Keep comparative coverage, performance, RSS, exploitability, public role projection, decoder, and concurrency claims unauthorized.
...
- Remove the generated ordered-pair binary that conflicted with the tracked-source boundary and reserve its path as generated output.
...
- Freeze a private 36-query ABI-role contract with 24 development queries, 12 source-disjoint confirmation queries, and 96 disabled-output closures that must preserve unchanged public text and JSON; add no public field, semantic class, score, schema, capacity, decoder, or worker change.
- Preserve the sealed P079 lifecycle prefix of 20 roots, 30 leaves, 29 aliases, two folds, 15 tombstones, 161 lineage records, and 87 events; P080 through P083 remain explicit unsealed deltas, and Patch 084 authorizes neither event 88 nor a new canonical ID.
...
- Record Patch 082 as not accepted and make Patch 083 the historical
  implementation candidate that did not complete acceptance; Patch 084 carried
  its bounded corrections.
...
- Add a test-only manifest for 30 ordered two-pop pairs. It declares existing
  and proposed correctness with zero incremental gains across 24 development
  and six confirmation-labeled tasks. The smoke validates the structure and
  declarations but does not execute an independent task consumer, so the result
  records a policy deferral rather than confirmatory measured task-value
  evidence.
- Retain the complete score/null partition by toggling 25 authority rows
  against two distinct static fixtures, yielding 50 deterministic check
  failures: 14 scored patterns, 11 null patterns, and three private role facets
  retained null.
...
- Sprint 13 Patch 078 corrects the remaining Patch 077 repository-binding,
  ...
  but Patches 083 through 088 did not complete acceptance. Patch 089 is the
  current corrective candidate. Public-field and score deferral remains
  unchanged, and independent P089 acceptance is pending.
...
- Patches 070, 071, 072, and 073 were not accepted at their respective first
  ...
  entry candidate. Patches 081 through 088 did not complete acceptance. Patch
  089 is the current exact-source implementation candidate, pending its fresh
  NASM/native, Docker, replay, split-debug, workload, both parity planes, delivery, and
  independent gates.
