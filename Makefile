...
CANDIDATE_ROLE_VECTOR_PROBE_BIN := $(INTERNAL_TEST_BUILD_DIR)/candidate-role-vector-probe
...
S13_EXPECTED_CANDIDATE_TREE ?=
S13_NATURAL_COORDINATE_RESULT_DIR ?=
S13_NATURAL_REPLAY_INPUT_DIR ?=
S13_NATURAL_REPLAY_RESULT_DIR ?=
S13_NATURAL_ATTRIBUTION_RESULT ?=
S13_ABI_ROLE_RESULT_DIR ?=
S13_ABI_ROLE_VECTOR_RESULT_DIR ?=
S13_WORKLOAD_PHASE_RESULT ?=
S13_SPLIT_DEBUG_PRODUCER_DIR ?=
S13_SPLIT_DEBUG_RESULT_DIR ?=
S13_WORKLOAD_REFERENCE_PREFLIGHT_RESULT_DIR ?=
...
.PHONY: sprint13-producer-authority-smoke sprint13-positive-coordinate-anchor-smoke sprint13-natural-coordinate-campaign-smoke sprint13-natural-coordinate-campaign sprint13-natural-coordinate-campaign-structural sprint13-natural-coordinate-campaign-run sprint13-natural-frozen-replay-smoke sprint13-natural-frozen-replay sprint13-natural-terminal-attribution-smoke sprint13-abi-role-query-contract-smoke sprint13-abi-role-query-smoke sprint13-lifecycle-denominator-smoke patch081-corrective-regression-smoke patch082-corrective-regression-smoke patch083-corrective-regression-smoke patch084-corrective-regression-smoke patch082-docker-corrective-regression-smoke sprint13-p082-acceptance-smoke sprint13-p083-acceptance-smoke sprint13-p084-acceptance-smoke sprint13-p085-acceptance-smoke sprint13-p086-acceptance-smoke patch085-corrective-regression-smoke sprint13-natural-frozen-replay-v2-smoke sprint13-natural-terminal-attribution-v2-smoke sprint13-natural-frozen-replay-v2 sprint13-abi-role-vector-equivalence-contract-smoke sprint13-abi-role-vector-equivalence-smoke docker-run-root-smoke patch086-corrective-regression-smoke sprint13-workload-phase-attribution-smoke sprint13-p087-acceptance-smoke patch087-corrective-regression-smoke sprint13-split-debug-packaging-contract-smoke sprint13-split-debug-packaging-smoke sprint13-p088-acceptance-smoke patch088-corrective-regression-smoke sprint13-p089-acceptance-smoke patch089-corrective-regression-smoke sprint13-workload-reference-preflight-contract-smoke sprint13-workload-reference-preflight-smoke sprint13-p090-acceptance-smoke
...
help:
	...
	@echo "  make patch086-corrective-regression-smoke  Regress Patch 086 transaction, replay, attribution, ABI, recovery, custody, and delivery findings"
	@echo "  make sprint13-workload-phase-attribution-smoke  Validate the bounded P087 paired phase-attribution experiment authority"
	@echo "  make sprint13-p087-acceptance-smoke  Run complete Patch 087 native, Docker, replay, parity, ABI, phase-authority, and strict-lint acceptance"
	@echo "  make patch087-corrective-regression-smoke  Regress Patch 087 transaction, recovery, replay, ABI, oracle, and delivery findings"
	@echo "  make sprint13-split-debug-packaging-contract-smoke  Validate the bounded two-build split-debug experiment contract"
	@echo "  make sprint13-split-debug-packaging-smoke  Execute the two-build split-debug experiment from retained producer builds"
	@echo "  make sprint13-p088-acceptance-smoke  Run complete Patch 088 native, Docker, replay, parity, split-debug, and strict-lint acceptance"
	@echo "  make patch088-corrective-regression-smoke  Regress Patch 088 replay, split-debug, workload, recovery, and delivery findings"
	@echo "  make patch089-corrective-regression-smoke  Regress Patch 089 authority, recovery, transaction, and delivery findings"
	@echo "  make sprint13-workload-reference-preflight-smoke  Run the 80-execution reference-only workload qualification preflight"
	@echo "  make sprint13-p089-acceptance-smoke  Run complete Patch 089 native, Docker, replay, parity, split-debug, workload, and strict-lint acceptance"
...
$(CANDIDATE_ROLE_VECTOR_PROBE_BIN): tests/internal/candidate-role-vector-probe.c $(BUILD_DIR)/candidate_role.o | $(INTERNAL_TEST_BUILD_DIR)
	$(CC) -std=c11 -Wall -Wextra -Werror -O2 -no-pie $^ -o $@
...
sprint13-producer-authority-smoke: build-tools-check
	...
	expected_tree="$(S13_EXPECTED_CANDIDATE_TREE)"; \
	if [ -z "$$expected_tree" ]; then expected_tree="$${X64LENS_CANDIDATE_TREE:-}"; fi; \
	if [ -z "$$expected_tree" ]; then \
		if git diff --quiet -- && git diff --cached --quiet -- && [ -z "$$(git ls-files --others --exclude-standard)" ]; then \
			expected_tree="$$(git rev-parse HEAD^{tree})"; \
		else \
			echo "error: an authenticated expected candidate tree is required for a non-clean source state" >&2; exit 2; \
		fi; \
	fi; \
	test "$${#expected_tree}" -eq 40 || { echo "error: invalid producer candidate tree" >&2; exit 2; }; \
	temporary_root=""; \
	cleanup() { \
	...
	python3 tools/sprint13-producer-authority-smoke.py run --repo . --result-dir "$$result_dir" --expected-candidate-tree "$$expected_tree"; \
	python3 tools/sprint13-ordered-two-pop-role-task-value-smoke.py \
		...
		--producer-manifest "$$result_dir/manifest.json" --expected-candidate-tree "$$expected_tree"; \
...
patch082-corrective-regression-smoke:
	python3 tools/patch082-corrective-regression-smoke.py

sprint13-natural-coordinate-campaign-smoke:
	python3 tools/sprint13-natural-coordinate-campaign.py selftest \
		--authority ./benchmarks/task-definitions/sprint13-natural-positive-coordinate-v1.json \
		--expected ./tests/expected/sprint13-natural-positive-coordinate-v1.json

sprint13-natural-coordinate-campaign: S13_NATURAL_REQUIRE_FLAG=--require-complete
sprint13-natural-coordinate-campaign: sprint13-natural-coordinate-campaign-run

sprint13-natural-coordinate-campaign-structural: S13_NATURAL_REQUIRE_FLAG=--require-structural-complete
sprint13-natural-coordinate-campaign-structural: sprint13-natural-coordinate-campaign-run

patch085-corrective-regression-smoke:
	python3 tools/patch085-corrective-regression-smoke.py

patch086-corrective-regression-smoke:
	python3 tools/patch086-corrective-regression-smoke.py

sprint13-workload-phase-attribution-smoke:
	python3 tools/sprint13-workload-phase-attribution-smoke.py selftest \
		--authority ./benchmarks/task-definitions/sprint13-workload-phase-attribution-v1.json \
		--expected ./tests/expected/sprint13-workload-phase-attribution-v1.json

sprint13-natural-frozen-replay-v2-smoke:
	python3 tools/sprint13-natural-frozen-replay-v2-smoke.py selftest \
		--authority ./benchmarks/task-definitions/sprint13-natural-frozen-replay-v2.json


sprint13-natural-terminal-attribution-v2-smoke:
	python3 tools/sprint13-natural-terminal-attribution-v2-smoke.py selftest \
		--authority ./benchmarks/task-definitions/sprint13-natural-frozen-replay-v2.json \
		--expected ./tests/expected/sprint13-natural-terminal-attribution-v2.json

sprint13-natural-frozen-replay-v2: all sprint13-natural-frozen-replay-v2-smoke sprint13-natural-terminal-attribution-v2-smoke
	@set -eu; \
	test -n "$(S13_NATURAL_REPLAY_INPUT_DIR)" || { echo "error: S13_NATURAL_REPLAY_INPUT_DIR is required" >&2; exit 2; }; \
	test -n "$(S13_NATURAL_REPLAY_RESULT_DIR)" || { echo "error: S13_NATURAL_REPLAY_RESULT_DIR is required" >&2; exit 2; }; \
	test -n "$(S13_NATURAL_ATTRIBUTION_RESULT)" || { echo "error: S13_NATURAL_ATTRIBUTION_RESULT is required" >&2; exit 2; }; \
	for tool in "$(ROPGADGET)" "$(ROPPER)" "$(ROPR)"; do command -v "$$tool" >/dev/null 2>&1 || { echo "error: required P087 replay tool is unavailable: $$tool" >&2; exit 127; }; done; \
	expected_tree="$(S13_EXPECTED_CANDIDATE_TREE)"; \
	test -n "$$expected_tree" || { echo "error: P087 replay requires an authenticated candidate tree" >&2; exit 2; }; \
	work="$$(mktemp -d "$${TMPDIR:-/tmp}/x64lens-p087-replay-source.XXXXXX")"; \
	work_identity="$$(python3 tools/remove-owned-tree.py --identify "$$work")"; \
	cleanup() { python3 tools/remove-owned-tree.py --remove "$$work" --identity "$$work_identity"; }; trap cleanup EXIT; \
	python3 tools/gitless-source-manifest.py create --repo . --root "$$work/source" --manifest "$$work/source-manifest.json" --expected-candidate-tree "$$expected_tree"; \
	python3 tools/sprint13-natural-frozen-replay-v2-smoke.py run \
		--authority ./benchmarks/task-definitions/sprint13-natural-frozen-replay-v2.json \
		--input-dir "$(S13_NATURAL_REPLAY_INPUT_DIR)" --result-dir "$(S13_NATURAL_REPLAY_RESULT_DIR)" \
		--x64lens ./$(TARGET) --ropgadget "$$(command -v "$(ROPGADGET)")" --ropper "$$(command -v "$(ROPPER)")" --ropr "$$(command -v "$(ROPR)")" \
		--source-root "$$work/source" --source-manifest "$$work/source-manifest.json" --expected-candidate-tree "$$expected_tree"; \
	python3 tools/sprint13-natural-terminal-attribution-v2-smoke.py run \
		--authority ./benchmarks/task-definitions/sprint13-natural-frozen-replay-v2.json \
		--expected ./tests/expected/sprint13-natural-terminal-attribution-v2.json \
		--input-dir "$(S13_NATURAL_REPLAY_RESULT_DIR)" --output "$(S13_NATURAL_ATTRIBUTION_RESULT)"

sprint13-abi-role-vector-equivalence-contract-smoke:
	python3 tools/sprint13-abi-role-vector-equivalence-smoke.py selftest

sprint13-abi-role-vector-equivalence-smoke: all $(CANDIDATE_ROLE_VECTOR_PROBE_BIN) sprint13-abi-role-vector-equivalence-contract-smoke
	@set -eu; \
	test -n "$(S13_ABI_ROLE_VECTOR_RESULT_DIR)" || { echo "error: S13_ABI_ROLE_VECTOR_RESULT_DIR is required" >&2; exit 2; }; \
	expected_tree="$(S13_EXPECTED_CANDIDATE_TREE)"; \
	test -n "$$expected_tree" || { echo "error: ABI role vector gate requires an authenticated candidate tree" >&2; exit 2; }; \
	work="$$(mktemp -d "$${TMPDIR:-/tmp}/x64lens-p087-vector-source.XXXXXX")"; \
	work_identity="$$(python3 tools/remove-owned-tree.py --identify "$$work")"; \
	cleanup() { python3 tools/remove-owned-tree.py --remove "$$work" --identity "$$work_identity"; }; trap cleanup EXIT; \
	python3 tools/gitless-source-manifest.py create --repo . --root "$$work/source" --manifest "$$work/source-manifest.json" --expected-candidate-tree "$$expected_tree"; \
	python3 tools/sprint13-abi-role-vector-equivalence-smoke.py run \
		--analyzer ./$(TARGET) --probe $(CANDIDATE_ROLE_VECTOR_PROBE_BIN) \
		--source-root "$$work/source" --source-manifest "$$work/source-manifest.json" --expected-candidate-tree "$$expected_tree" \
		--result-dir "$(S13_ABI_ROLE_VECTOR_RESULT_DIR)"

sprint13-natural-frozen-replay-smoke:
	python3 tools/sprint13-natural-frozen-replay-smoke.py selftest \
		--authority ./benchmarks/task-definitions/sprint13-natural-frozen-replay-v1.json

sprint13-natural-terminal-attribution-smoke:
	python3 tools/sprint13-natural-terminal-attribution-smoke.py selftest \
		--authority ./benchmarks/task-definitions/sprint13-natural-frozen-replay-v1.json

sprint13-natural-frozen-replay: all sprint13-natural-frozen-replay-smoke sprint13-natural-terminal-attribution-smoke
	@set -eu; \
	test -n "$(S13_NATURAL_REPLAY_INPUT_DIR)" || { echo "error: S13_NATURAL_REPLAY_INPUT_DIR is required" >&2; exit 2; }; \
	test -n "$(S13_NATURAL_REPLAY_RESULT_DIR)" || { echo "error: S13_NATURAL_REPLAY_RESULT_DIR is required" >&2; exit 2; }; \
	test -n "$(S13_NATURAL_ATTRIBUTION_RESULT)" || { echo "error: S13_NATURAL_ATTRIBUTION_RESULT is required" >&2; exit 2; }; \
	for tool in "$(ROPGADGET)" "$(ROPPER)" "$(ROPR)"; do command -v "$$tool" >/dev/null 2>&1 || { echo "error: required frozen-replay tool is unavailable: $$tool" >&2; exit 127; }; done; \
	expected_tree="$(S13_EXPECTED_CANDIDATE_TREE)"; \
	if [ -z "$$expected_tree" ]; then expected_tree="$${X64LENS_CANDIDATE_TREE:-}"; fi; \
	test -n "$$expected_tree" || { echo "error: an authenticated expected candidate tree is required" >&2; exit 2; }; \
	work="$$(mktemp -d "$${TMPDIR:-/tmp}/x64lens-natural-replay-source.XXXXXX")"; \
	work_identity="$$(python3 tools/remove-owned-tree.py --identify "$$work")"; \
	cleanup() { python3 tools/remove-owned-tree.py --remove "$$work" --identity "$$work_identity"; }; \
	trap cleanup EXIT; \
	python3 tools/gitless-source-manifest.py create \
		--repo . --root "$$work/source" --manifest "$$work/source-manifest.json" \
		--expected-candidate-tree "$$expected_tree"; \
	python3 tools/sprint13-natural-frozen-replay-smoke.py run \
		--authority ./benchmarks/task-definitions/sprint13-natural-frozen-replay-v1.json \
		--input-dir "$(S13_NATURAL_REPLAY_INPUT_DIR)" \
		--result-dir "$(S13_NATURAL_REPLAY_RESULT_DIR)" \
		--x64lens ./$(TARGET) \
		--ropgadget "$$(command -v "$(ROPGADGET)")" \
		--ropper "$$(command -v "$(ROPPER)")" \
		--ropr "$$(command -v "$(ROPR)")" \
		--source-root "$$work/source" --source-manifest "$$work/source-manifest.json" \
		--expected-candidate-tree "$$expected_tree"; \
	python3 tools/sprint13-natural-terminal-attribution-smoke.py run \
		--authority ./benchmarks/task-definitions/sprint13-natural-frozen-replay-v1.json \
		--input-dir "$(S13_NATURAL_REPLAY_RESULT_DIR)" \
		--output "$(S13_NATURAL_ATTRIBUTION_RESULT)"

sprint13-natural-coordinate-campaign-run: all
	@set -eu; \
	test -n "$(S13_NATURAL_COORDINATE_RESULT_DIR)" || { echo "error: S13_NATURAL_COORDINATE_RESULT_DIR is required" >&2; exit 2; }; \
	for tool in "$(ROPGADGET)" "$(ROPPER)" "$(ROPR)" "$(READELF)" "$(DPKG_QUERY)"; do command -v "$$tool" >/dev/null 2>&1 || { echo "error: required natural-coordinate tool is unavailable: $$tool" >&2; exit 127; }; done; \
	expected_tree="$(S13_EXPECTED_CANDIDATE_TREE)"; \
	if [ -z "$$expected_tree" ]; then expected_tree="$${X64LENS_CANDIDATE_TREE:-}"; fi; \
	test -n "$$expected_tree" || { echo "error: an authenticated expected candidate tree is required" >&2; exit 2; }; \
	work="$$(mktemp -d "$${TMPDIR:-/tmp}/x64lens-natural-source.XXXXXX")"; \
	work_identity="$$(python3 tools/remove-owned-tree.py --identify "$$work")"; \
	cleanup() { python3 tools/remove-owned-tree.py --remove "$$work" --identity "$$work_identity"; }; \
	trap cleanup EXIT; \
	python3 tools/gitless-source-manifest.py create \
		--repo . --root "$$work/source" --manifest "$$work/source-manifest.json" \
		--expected-candidate-tree "$$expected_tree"; \
	python3 tools/sprint13-natural-coordinate-campaign.py run \
		--authority ./benchmarks/task-definitions/sprint13-natural-positive-coordinate-v1.json \
		--result-dir "$(S13_NATURAL_COORDINATE_RESULT_DIR)" \
		--x64lens ./$(TARGET) \
		--ropgadget "$$(command -v "$(ROPGADGET)")" \
		--ropper "$$(command -v "$(ROPPER)")" \
		--ropr "$$(command -v "$(ROPR)")" \
		...
		--expected-candidate-tree "$$expected_tree" \
		$(S13_NATURAL_REQUIRE_FLAG)

patch083-corrective-regression-smoke:
	python3 tools/patch083-corrective-regression-smoke.py

patch084-corrective-regression-smoke:
	python3 tools/patch084-corrective-regression-smoke.py

sprint13-lifecycle-denominator-smoke:
	python3 tools/sprint13-lifecycle-denominator-smoke.py \
		--authority ./benchmarks/task-definitions/sprint13-lifecycle-denominator-authority-v1.json \
		--expected ./tests/expected/sprint13-lifecycle-denominator-authority-v1.json

sprint13-abi-role-query-contract-smoke:
	python3 tools/sprint13-abi-role-query-smoke.py contract \
		--authority ./benchmarks/task-definitions/sprint13-abi-role-query-v1.json \
		--expected ./tests/expected/sprint13-abi-role-query-v1.json

sprint13-abi-role-query-smoke: all
	@set -eu; \
	expected_tree="$(S13_EXPECTED_CANDIDATE_TREE)"; \
	if [ -z "$$expected_tree" ]; then expected_tree="$${X64LENS_CANDIDATE_TREE:-}"; fi; \
	test -n "$$expected_tree" || { echo "error: an authenticated expected candidate tree is required" >&2; exit 2; }; \
	work="$$(mktemp -d "$${TMPDIR:-/tmp}/x64lens-abi-role.XXXXXX")"; \
	work_identity="$$(python3 tools/remove-owned-tree.py --identify "$$work")"; \
	result_dir="$(S13_ABI_ROLE_RESULT_DIR)"; \
	cleanup() { python3 tools/remove-owned-tree.py --remove "$$work" --identity "$$work_identity"; }; \
	trap cleanup EXIT; \
	python3 tools/gitless-source-manifest.py create \
		--repo . --root "$$work/source" --manifest "$$work/source-manifest.json" \
		--expected-candidate-tree "$$expected_tree"; \
	if [ -z "$$result_dir" ]; then result_dir="$$work/result"; fi; \
	python3 tools/sprint13-abi-role-query-smoke.py run \
		--authority ./benchmarks/task-definitions/sprint13-abi-role-query-v1.json \
		--expected ./tests/expected/sprint13-abi-role-query-v1.json \
		--analyzer ./$(TARGET) --result-dir "$$result_dir" \
		--source-root "$$work/source" --source-manifest "$$work/source-manifest.json" \
		--expected-candidate-tree "$$expected_tree"; \
	if [ -n "$(S13_ABI_ROLE_RESULT_DIR)" ]; then echo "ABI-role query result retained: $$result_dir"; fi
...
sprint13-p082-acceptance-smoke: S13_EXPECTED_CANDIDATE_TREE := 112ce326715975dbdfecfa3b02c2ce25abb6be3f
...
# Patch 083 closes the localized P082 transaction, producer, Docker, corpus,
# recovery, and delivery defects and executes the outcome-blind natural
# coordinate campaign.  It changes no analyzer runtime, public field, score,
# or schema meaning.
sprint13-p083-acceptance-smoke: S13_EXPECTED_CANDIDATE_TREE := 39671c27342a1c093de2e37806e58bdf209677d2
sprint13-p083-acceptance-smoke: validation-smoke docker-validation-smoke patch082-docker-corrective-regression-smoke sprint13-natural-coordinate-campaign sprint12-external-natural-acquisition-smoke sprint12-role-property-environment-parity-smoke sprint12-dynamic-metadata-environment-parity-smoke sprint12-role-property-public-policy-smoke sprint12-mitigation-competitive-gap-smoke sprint13-producer-authority-smoke docker-image-authority-smoke
	@command -v "$(SHELLCHECK)" >/dev/null 2>&1 || { \
		echo "error: sprint13-p083-acceptance-smoke requires $(SHELLCHECK)" >&2; \
		exit 127; \
	}
	@SHELLCHECK_STRICT=1 $(MAKE) --no-print-directory shellcheck-smoke
	@echo "sprint13-p083-acceptance-smoke: ok patch=83 sprint12=closed sprint13=active natural-coordinate-campaign=complete public-fields-added=0 semantic-changes=0 score-changes=0 schema=0.2.0"

# Patch 084 reconciles the committed P083 source, closes the natural-campaign,
# transaction, recovery, Docker, source-custody, and delivery findings, freezes
# the private ABI-role query contract, and preserves lifecycle denominators.
# Natural terminal outcomes remain diagnostic; comparison qualification is not
# required for product-patch acceptance and authorizes no comparative claim.
sprint13-p084-acceptance-smoke: S13_EXPECTED_CANDIDATE_TREE := 178f1d1c93a5a05fba5a77af2c378e63e5dc017b
sprint13-p084-acceptance-smoke: validation-smoke docker-validation-smoke patch083-corrective-regression-smoke sprint13-lifecycle-denominator-smoke sprint13-abi-role-query-smoke sprint13-natural-coordinate-campaign-structural sprint12-external-natural-acquisition-smoke sprint12-role-property-environment-parity-smoke sprint12-dynamic-metadata-environment-parity-smoke sprint13-producer-authority-smoke docker-image-authority-smoke
	@command -v "$(SHELLCHECK)" >/dev/null 2>&1 || { \
		echo "error: sprint13-p084-acceptance-smoke requires $(SHELLCHECK)" >&2; \
		exit 127; \
	}
	@SHELLCHECK_STRICT=1 $(MAKE) --no-print-directory shellcheck-smoke
	@echo "sprint13-p084-acceptance-smoke: ok patch=84 sprint12=closed sprint13=active natural-coordinate-campaign=terminal-diagnostic abi-role-queries=36 public-closures=96 lifecycle-prefix=preserved public-fields-added=0 semantic-changes=0 score-changes=0 schema=0.2.0"

# Patch 085 closes the P084 transaction, recovery, Git-less custody, ABI,
# natural-campaign, fixed-tree, evidence, and delivery findings.  It replays
# the exact frozen natural targets without reroll and attributes terminal
# outcomes by layer.  Runtime analyzer, public output, schema, semantics, and
# score policy remain unchanged.
sprint13-p085-acceptance-smoke: S13_EXPECTED_CANDIDATE_TREE := b2d2549a4ec311d97e79925035f80d7535867ac0
sprint13-p085-acceptance-smoke: validation-smoke docker-validation-smoke patch084-corrective-regression-smoke sprint13-lifecycle-denominator-smoke sprint13-abi-role-query-smoke sprint13-natural-frozen-replay sprint12-external-natural-acquisition-smoke sprint12-role-property-environment-parity-smoke sprint12-dynamic-metadata-environment-parity-smoke sprint13-producer-authority-smoke docker-image-authority-smoke
	@set -eu; \
	expected_tree="$(S13_EXPECTED_CANDIDATE_TREE)"; \
	if [ -z "$$expected_tree" ]; then expected_tree="$${X64LENS_CANDIDATE_TREE:-}"; fi; \
	test -n "$$expected_tree" || { echo "error: sprint13-p085-acceptance-smoke requires an authenticated candidate tree" >&2; exit 2; }; \
	test "$${#expected_tree}" -eq 40 || { echo "error: invalid P085 candidate tree" >&2; exit 2; }
	@command -v "$(SHELLCHECK)" >/dev/null 2>&1 || { \
		echo "error: sprint13-p085-acceptance-smoke requires $(SHELLCHECK)" >&2; \
		exit 127; \
	}
	@SHELLCHECK_STRICT=1 $(MAKE) --no-print-directory shellcheck-smoke
	@echo "sprint13-p085-acceptance-smoke: ok patch=85 sprint12=closed sprint13=active frozen-replay=complete terminal-attribution=layered lifecycle-deltas=5 abi-role-queries=36 public-closures=96 public-fields-added=0 semantic-changes=0 score-changes=0 schema=0.2.0"

# Patch 086 closes the P085 replay/oracle, hard-link, catchable-signal,
# source-custody, ABI-expected, and delivery findings.  It additionally
# proves private production-vector equivalence without changing public output.
sprint13-p086-acceptance-smoke: validation-smoke docker-validation-smoke patch085-corrective-regression-smoke sprint13-lifecycle-denominator-smoke sprint13-abi-role-vector-equivalence-smoke sprint13-natural-frozen-replay-v2 sprint12-external-natural-acquisition-smoke sprint12-role-property-environment-parity-smoke sprint12-dynamic-metadata-environment-parity-smoke sprint13-producer-authority-smoke docker-image-authority-smoke
	@set -eu; \
	expected_tree="$(S13_EXPECTED_CANDIDATE_TREE)"; \
	test -n "$$expected_tree" || { echo "error: sprint13-p086-acceptance-smoke requires an authenticated candidate tree" >&2; exit 2; }; \
	test "$${#expected_tree}" -eq 40 || { echo "error: invalid P086 candidate tree" >&2; exit 2; }
	@command -v "$(SHELLCHECK)" >/dev/null 2>&1 || { echo "error: sprint13-p086-acceptance-smoke requires $(SHELLCHECK)" >&2; exit 127; }
	@SHELLCHECK_STRICT=1 $(MAKE) --no-print-directory shellcheck-smoke
	@echo "sprint13-p086-acceptance-smoke: ok patch=86 sprint12=closed sprint13=active frozen-replay=sealed terminal-attribution=expected abi-vector-equivalence=private public-fields-added=0 semantic-changes=0 score-changes=0 schema=0.2.0"

# Patch 087 closes the P086 transaction, replay-runtime, terminal, ABI-publication,
# recovery, custody, and loose-delivery findings. It freezes the paired workload/
# phase-attribution experiment without selecting an optimization or changing runtime facts.
sprint13-p087-acceptance-smoke: validation-smoke docker-validation-smoke patch086-corrective-regression-smoke sprint13-lifecycle-denominator-smoke sprint13-abi-role-vector-equivalence-smoke sprint13-natural-frozen-replay-v2 sprint13-workload-phase-attribution-smoke sprint12-external-natural-acquisition-smoke sprint12-role-property-environment-parity-smoke sprint12-dynamic-metadata-environment-parity-smoke sprint13-producer-authority-smoke docker-image-authority-smoke
	@set -eu; \
	expected_tree="$(S13_EXPECTED_CANDIDATE_TREE)"; \
	test -n "$$expected_tree" || { echo "error: sprint13-p087-acceptance-smoke requires an authenticated candidate tree" >&2; exit 2; }; \
	test "$${#expected_tree}" -eq 40 || { echo "error: invalid P087 candidate tree" >&2; exit 2; }
	@command -v "$(SHELLCHECK)" >/dev/null 2>&1 || { echo "error: sprint13-p087-acceptance-smoke requires $(SHELLCHECK)" >&2; exit 127; }
	@SHELLCHECK_STRICT=1 $(MAKE) --no-print-directory shellcheck-smoke
	@echo "sprint13-p087-acceptance-smoke: ok patch=87 sprint12=closed sprint13=active frozen-replay=sealed terminal-attribution=expected abi-vector-equivalence=private workload-phase-authority=frozen public-fields-added=0 semantic-changes=0 score-changes=0 schema=0.2.0"

patch087-corrective-regression-smoke:
	python3 tools/patch087-corrective-regression-smoke.py

patch088-corrective-regression-smoke:
	python3 tools/patch088-corrective-regression-smoke.py

sprint13-split-debug-packaging-contract-smoke:
	python3 tools/sprint13-split-debug-packaging-smoke.py selftest \
		--authority ./benchmarks/task-definitions/sprint13-split-debug-packaging-v1.json \
		--expected ./tests/expected/sprint13-split-debug-packaging-v1.json

sprint13-split-debug-packaging-smoke: build-tools-check sprint13-split-debug-packaging-contract-smoke
	@set -eu; \
	expected_tree="$(S13_EXPECTED_CANDIDATE_TREE)"; \
	if [ -z "$$expected_tree" ]; then expected_tree="$${X64LENS_CANDIDATE_TREE:-}"; fi; \
	if [ -z "$$expected_tree" ]; then \
		if git diff --quiet -- && git diff --cached --quiet -- && [ -z "$$(git ls-files --others --exclude-standard)" ]; then expected_tree="$$(git rev-parse HEAD^{tree})"; \
		else echo "error: split-debug execution requires an authenticated candidate tree" >&2; exit 2; fi; \
	fi; \
	test "$${#expected_tree}" -eq 40 || { echo "error: invalid split-debug candidate tree" >&2; exit 2; }; \
	temporary_root="$$(mktemp -d "$${TMPDIR:-/tmp}/x64lens-p089-split-debug.XXXXXX")"; \
	temporary_identity="$$(python3 tools/remove-owned-tree.py --identify "$$temporary_root")"; \
	cleanup() { python3 tools/remove-owned-tree.py --remove "$$temporary_root" --identity "$$temporary_identity"; }; \
	trap cleanup EXIT; trap 'exit 129' HUP; trap 'exit 130' INT; trap 'exit 143' TERM; \
	producer="$(S13_SPLIT_DEBUG_PRODUCER_DIR)"; \
	if [ -z "$$producer" ]; then producer="$$temporary_root/producer"; fi; \
	result="$(S13_SPLIT_DEBUG_RESULT_DIR)"; \
	if [ -z "$$result" ]; then result="$$temporary_root/result"; fi; \
	if [ ! -f "$$producer/manifest.json" ]; then \
		S13_PRODUCER_RESULT_DIR="$$producer" S13_EXPECTED_CANDIDATE_TREE="$$expected_tree" $(MAKE) --no-print-directory sprint13-producer-authority-smoke; \
	fi; \
	python3 tools/sprint13-split-debug-packaging-smoke.py run \
		--authority ./benchmarks/task-definitions/sprint13-split-debug-packaging-v1.json \
		--expected ./tests/expected/sprint13-split-debug-packaging-v1.json \
		--producer-root "$$producer" --producer-manifest "$$producer/manifest.json" \
		--result-dir "$$result" --expected-source-tree "$$expected_tree"; \
	python3 tools/sprint13-split-debug-packaging-smoke.py verify \
		--authority ./benchmarks/task-definitions/sprint13-split-debug-packaging-v1.json \
		--expected ./tests/expected/sprint13-split-debug-packaging-v1.json \
		--producer-root "$$producer" --producer-manifest "$$producer/manifest.json" \
		--result-dir "$$result" --expected-source-tree "$$expected_tree"; \
	if [ -n "$(S13_SPLIT_DEBUG_RESULT_DIR)" ]; then echo "split-debug retained result: $$result"; fi

# Patch 088 closes the P087 transaction, replay-runtime, recovery, ABI-stage,
# workload-oracle, source-identity, evidence, and delivery findings. It executes
# only the bounded two-build split-debug experiment; no runtime or public product
# policy changes are authorized.
sprint13-p088-acceptance-smoke: validation-smoke docker-validation-smoke patch087-corrective-regression-smoke sprint13-lifecycle-denominator-smoke sprint13-abi-role-vector-equivalence-smoke sprint13-natural-frozen-replay-v2 sprint13-workload-phase-attribution-smoke sprint13-split-debug-packaging-smoke sprint12-external-natural-acquisition-smoke sprint12-role-property-environment-parity-smoke sprint12-dynamic-metadata-environment-parity-smoke sprint13-producer-authority-smoke docker-image-authority-smoke
	@set -eu; expected_tree="$(S13_EXPECTED_CANDIDATE_TREE)"; \
	test -n "$$expected_tree" || { echo "error: sprint13-p088-acceptance-smoke requires an authenticated candidate tree" >&2; exit 2; }; \
	test "$${#expected_tree}" -eq 40 || { echo "error: invalid P088 candidate tree" >&2; exit 2; }
	@command -v "$(SHELLCHECK)" >/dev/null 2>&1 || { echo "error: sprint13-p088-acceptance-smoke requires $(SHELLCHECK)" >&2; exit 127; }
	@SHELLCHECK_STRICT=1 $(MAKE) --no-print-directory shellcheck-smoke
	@echo "sprint13-p088-acceptance-smoke: ok patch=88 sprint12=closed sprint13=active frozen-replay=sealed terminal-attribution=expected abi-vector-equivalence=private workload-phase-authority=frozen split-debug=diagnostic product-adoption=0 public-fields-added=0 semantic-changes=0 score-changes=0 schema=0.2.0"

# Patch 089 closes the P088 replay-identity, Python-closure, split-debug
# result-custody, workload-fixture, streaming-recovery, oracle, and delivery
# findings. Split-debug remains an opt-in diagnostic experiment and does not
# become the default product package.
sprint13-p089-acceptance-smoke: validation-smoke docker-validation-smoke patch088-corrective-regression-smoke sprint13-lifecycle-denominator-smoke sprint13-abi-role-vector-equivalence-smoke sprint13-natural-frozen-replay-v2 sprint13-workload-phase-attribution-smoke sprint13-split-debug-packaging-smoke sprint12-external-natural-acquisition-smoke sprint12-role-property-environment-parity-smoke sprint12-dynamic-metadata-environment-parity-smoke sprint13-producer-authority-smoke docker-image-authority-smoke
	@set -eu; expected_tree="$(S13_EXPECTED_CANDIDATE_TREE)"; \
	test -n "$$expected_tree" || { echo "error: sprint13-p089-acceptance-smoke requires an authenticated candidate tree" >&2; exit 2; }; \
	test "$${#expected_tree}" -eq 40 || { echo "error: invalid P089 candidate tree" >&2; exit 2; }
	@command -v "$(SHELLCHECK)" >/dev/null 2>&1 || { echo "error: sprint13-p089-acceptance-smoke requires $(SHELLCHECK)" >&2; exit 127; }
	@SHELLCHECK_STRICT=1 $(MAKE) --no-print-directory shellcheck-smoke
	@echo "sprint13-p089-acceptance-smoke: ok patch=89 sprint12=closed sprint13=active replay-predecessor-identity=separate python-closures=exact workload-fixtures=unique split-debug=opt-in-diagnostic path-stable=0 total-transfer-reduction=0 product-adoption=0 public-fields-added=0 semantic-changes=0 score-changes=0 schema=0.2.0"

patch089-corrective-regression-smoke:
	python3 tools/patch089-corrective-regression-smoke.py

sprint13-workload-reference-preflight-contract-smoke:
	python3 tools/sprint13-workload-reference-preflight-smoke.py selftest \
		--authority ./benchmarks/task-definitions/sprint13-workload-reference-preflight-v1.json \
		--expected ./tests/expected/sprint13-workload-reference-preflight-v1.json

# The P090 workload gate is reference-only. Its 80 executions qualify or retire
# the later paired 160-run phase-attribution matrix, but none of these rows may
# be reused in that matrix or interpreted as a performance result.
sprint13-workload-reference-preflight-smoke: all samples sprint13-workload-reference-preflight-contract-smoke
	@set -eu; \
	expected_tree="$(S13_EXPECTED_CANDIDATE_TREE)"; \
	if [ -z "$$expected_tree" ]; then expected_tree="$${X64LENS_CANDIDATE_TREE:-}"; fi; \
	if [ -z "$$expected_tree" ]; then \
		if git diff --quiet -- && git diff --cached --quiet -- && [ -z "$$(git ls-files --others --exclude-standard)" ]; then expected_tree="$$(git rev-parse HEAD^{tree})"; \
		else echo "error: reference preflight requires an authenticated candidate tree" >&2; exit 2; fi; \
	fi; \
	test "$${#expected_tree}" -eq 40 || { echo "error: invalid P090 candidate tree" >&2; exit 2; }; \
	temporary_root="$$(mktemp -d "$${TMPDIR:-/tmp}/x64lens-p090-reference-preflight.XXXXXX")"; \
	trap 'rm -rf "$$temporary_root"' EXIT HUP INT TERM; \
	source_root="$$temporary_root/source"; \
	source_manifest="$$temporary_root/source-manifest.json"; \
	python3 tools/gitless-source-manifest.py create --repo . --root "$$source_root" --manifest "$$source_manifest" --expected-candidate-tree "$$expected_tree"; \
	result="$(S13_WORKLOAD_REFERENCE_PREFLIGHT_RESULT_DIR)"; \
	if [ -z "$$result" ]; then result="$$temporary_root/result"; fi; \
	python3 tools/sprint13-workload-reference-preflight-smoke.py run \
		--authority ./benchmarks/task-definitions/sprint13-workload-reference-preflight-v1.json \
		--expected ./tests/expected/sprint13-workload-reference-preflight-v1.json \
		--source-root "$$source_root" --source-manifest "$$source_manifest" \
		--expected-candidate-tree "$$expected_tree" \
		--analyzer ./$(TARGET) --analyzer-source-tree "$$expected_tree" \
		--target-root . --result-dir "$$result" --evidence-class fresh; \
	python3 tools/sprint13-workload-reference-preflight-smoke.py verify \
		--authority ./benchmarks/task-definitions/sprint13-workload-reference-preflight-v1.json \
		--expected ./tests/expected/sprint13-workload-reference-preflight-v1.json \
		--source-root "$$source_root" --source-manifest "$$source_manifest" \
		--expected-candidate-tree "$$expected_tree" \
		--analyzer ./$(TARGET) --analyzer-source-tree "$$expected_tree" \
		--target-root . --result-dir "$$result" --evidence-class fresh; \
	if [ -n "$(S13_WORKLOAD_REFERENCE_PREFLIGHT_RESULT_DIR)" ]; then echo "reference preflight retained result: $$result"; fi

# Patch 090 closes the P089 authority, recovery, transaction, package, and
# evidence findings and runs only the bounded reference-workload preflight.
# It adds no public field, semantic class, score, schema change, decoder, or
# concurrency profile.
sprint13-p090-acceptance-smoke: validation-smoke docker-validation-smoke patch089-corrective-regression-smoke sprint13-workload-reference-preflight-smoke sprint13-split-debug-packaging-smoke sprint13-lifecycle-denominator-smoke sprint13-abi-role-vector-equivalence-smoke sprint13-natural-frozen-replay-v2 sprint12-external-natural-acquisition-smoke sprint12-role-property-environment-parity-smoke sprint12-dynamic-metadata-environment-parity-smoke sprint13-producer-authority-smoke docker-image-authority-smoke
	@set -eu; expected_tree="$(S13_EXPECTED_CANDIDATE_TREE)"; \
	test -n "$$expected_tree" || { echo "error: sprint13-p090-acceptance-smoke requires an authenticated candidate tree" >&2; exit 2; }; \
	test "$${#expected_tree}" -eq 40 || { echo "error: invalid P090 candidate tree" >&2; exit 2; }
	@command -v "$(SHELLCHECK)" >/dev/null 2>&1 || { echo "error: sprint13-p090-acceptance-smoke requires $(SHELLCHECK)" >&2; exit 127; }
	@SHELLCHECK_STRICT=1 $(MAKE) --no-print-directory shellcheck-smoke
	@echo "sprint13-p090-acceptance-smoke: ok patch=90 sprint12=closed sprint13=active reference-preflight=complete split-debug=opt-in-diagnostic product-adoption=0 public-fields-added=0 semantic-changes=0 score-changes=0 schema=0.2.0"
...
script-perms-check:
	...
	@test -x tools/docker-run-root-smoke.sh
	@test -x tools/sprint13-natural-coordinate-campaign.py
	@test -x tools/patch083-corrective-regression-smoke.py
	@test -x tools/patch084-corrective-regression-smoke.py
	@test -x tools/sprint13-natural-frozen-replay-smoke.py
	@test -x tools/sprint13-natural-terminal-attribution-smoke.py
	@test -x tools/sprint13-abi-role-query-smoke.py
	@test -x tools/sprint13-lifecycle-denominator-smoke.py
	@test -f benchmarks/task-definitions/sprint13-abi-role-query-v1.json
	@test -f benchmarks/task-definitions/sprint13-lifecycle-denominator-authority-v1.json
	@test -f benchmarks/task-definitions/sprint13-natural-frozen-replay-v1.json
	@test -f tests/expected/sprint13-natural-terminal-attribution-v1.json
	@test -f tests/expected/sprint13-abi-role-query-v1.json
	@test -f tests/expected/sprint13-lifecycle-denominator-authority-v1.json
	@test -f docs/adr/0070-patch083-correction-and-abi-role-contract.md
	@test -f docs/adr/0071-patch084-correction-frozen-replay-and-terminal-attribution.md
	@test -f docs/sprints/sprint-13-patch-084-validation.md
	@test -f docs/sprints/sprint-13-patch-085-validation.md
	@test -x tools/patch082-corrective-regression-smoke.py
...
scaffold-check: script-perms-check
	...
	@test -f tests/internal/candidate-role-vector-probe.c
	@test -f benchmarks/task-definitions/sprint13-abi-role-vector-equivalence-v1.json
	@test -f benchmarks/task-definitions/sprint13-natural-frozen-replay-v2.json
	@test -f tests/expected/sprint13-abi-role-vector-equivalence-v1.json
	@test -f tests/expected/sprint13-natural-terminal-attribution-v2.json
	@test -x tools/patch085-corrective-regression-smoke.py
	@test -x tools/sprint13-abi-role-vector-equivalence-smoke.py
	@test -x tools/sprint13-natural-frozen-replay-v2-smoke.py
	@test -x tools/sprint13-natural-terminal-attribution-v2-smoke.py
	@test -x tools/patch086-corrective-regression-smoke.py
	@test -x tools/sprint13-workload-phase-attribution-smoke.py
	@test -f benchmarks/task-definitions/sprint13-workload-phase-attribution-v1.json
	@test -f tests/expected/sprint13-workload-phase-attribution-v1.json
	@test -x tools/patch087-corrective-regression-smoke.py
	@test -x tools/patch088-corrective-regression-smoke.py
	@test -x tools/sprint13-split-debug-packaging-smoke.py
	@test -f benchmarks/task-definitions/sprint13-split-debug-packaging-v1.json
	@test -f tests/expected/sprint13-split-debug-packaging-v1.json
	...
	@test -f docs/adr/0075-patch088-correction-and-p089-evidence-gates.md
	@test -f docs/sprints/sprint-13-patch-089-validation.md
...
docker-build: docker-available-check
	...
	case "$$expected_tree" in \
		????????????????????????????????????????) ;; \
		*) echo "error: invalid expected Docker candidate tree" >&2; exit 2 ;; \
	esac; \
	python3 tools/gitless-source-manifest.py create-context --repo . --context "$$context" --expected-candidate-tree "$$expected_tree"; \
	...
	test "$$tree" = "$$expected_tree" || { echo "error: Docker context tree differs from expected candidate tree" >&2; exit 1; }; \
...
docker-run-root-smoke: docker-build
	@set -eu; \
	python3 tools/docker-image-authority.py verify --path "$(DOCKER_IMAGE_AUTHORITY)" --docker "$(DOCKER)"; \
	image_id="$$(python3 tools/docker-image-authority.py get --path "$(DOCKER_IMAGE_AUTHORITY)" --field image_id)"; \
	bash tools/docker-run-root-smoke.sh "$$image_id"

patch082-docker-corrective-regression-smoke: docker-run-root-smoke
	@echo "patch082-docker-corrective-regression-smoke: ok writable-run-root=1 callers=3 nonroot=1 source-pristine=1"
...
docker-test: docker-build
	...
	"$(DOCKER)" run --rm -e HOME=/tmp -e X64LENS_CANDIDATE_TREE="$$tree" \
		...
		bash -lc 'set -euo pipefail; run="$${X64LENS_RUN_ROOT:?}"; rm -rf "$$run"; mkdir "$$run"; cp -a /work/. "$$run"/; chmod -R u+w "$$run"; cd "$$run"; make clean; make; make test; python3 /work/tools/gitless-source-manifest.py verify --root /work --manifest /x64lens-source-manifest.json'
...
docker-validation-smoke: docker-build docker-context-hygiene-smoke docker-source-custody-smoke docker-run-root-smoke
	@set -eu; \
	python3 tools/docker-image-authority.py verify --path "$(DOCKER_IMAGE_AUTHORITY)" --docker "$(DOCKER)"; \
	image_id="$$(python3 tools/docker-image-authority.py get --path "$(DOCKER_IMAGE_AUTHORITY)" --field image_id)"; \
	tree="$$(python3 tools/docker-image-authority.py get --path "$(DOCKER_IMAGE_AUTHORITY)" --field candidate_tree)"; \
	"$(DOCKER)" run --rm -e HOME=/tmp -e X64LENS_CANDIDATE_TREE="$$tree" \
		-e X64LENS_SOURCE_MANIFEST=/x64lens-source-manifest.json \
		-e X64LENS_SOURCE_AUTHORITY_ROOT=/work "$$image_id" \
		bash -lc 'set -euo pipefail; run="$${X64LENS_RUN_ROOT:?}"; rm -rf "$$run"; mkdir "$$run"; cp -a /work/. "$$run"/; chmod -R u+w "$$run"; cd "$$run"; make clean; make; make validation-smoke; python3 /work/tools/gitless-source-manifest.py verify --root /work --manifest /x64lens-source-manifest.json'
