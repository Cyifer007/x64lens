...
Sprint 7 adds a compiler-independent program-header matrix, and Sprint 8 Patch 032 expands it to twenty valid layouts and twelve malformed layouts. The matrix fixes expected PIE, NX-stack, no/partial/full RELRO, canary-indicator, dynamic-linking, bind-now, dynamic-table count and terminator, RWX-load, load-count, executable-region behavior, selected table-end overflow rejection, bounded dynamic-table rejection, and duplicate dynamic-table rejection. These tests provide regression evidence for represented cases, not a proof of complete mitigation coverage or memory safety.
