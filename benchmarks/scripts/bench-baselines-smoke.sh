...
run_timed() {
  ...
  if [[ "$tool_name" == "x64lens" && "$status" -eq 0 ]]; then
    if ! python3 -m json.tool "$out" >/dev/null; then
      note="invalid_json"
      status=1
    elif ! python3 "$ROOT/tools/validate-json-report.py" --mode system --require-schema 0.2.0 --expected-command gadgets --require-provenance --require-sprint10-effects --require-sprint10-transfer --require-sprint10-memory --require-sprint10-architectural-effects "$out" >/dev/null; then
